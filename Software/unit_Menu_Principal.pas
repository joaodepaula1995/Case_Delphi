...
begin
  begin
    // Obter a data atual
    dia_atual := Now;
    // Obter o número do dia da semana (1 a 7)
    dia_semana := DayOfWeek(dia_atual);
    // Converter o número do dia da semana para o nome do dia
    case dia_semana of
      1:
        nome_do_dia := 'Domingo';
      2:
    ...
    lbl_dia_semana.Caption := 'Hoje é ' + Format('%s (%s)', [nome_do_dia, FormatDateTime('dd/mm/yyyy', dia_atual)]);
  end;
  // Iniciar o timer quando o formulário é criado
  tmr_hora.Enabled := True;
end;

procedure TFRM_MENU_PRINCIPAL.tmr_horaTimer(Sender: TObject);
begin
  lbl_hora.Caption := 'Horário atual: ' + FormatDateTime('hh:mm:ss', Now);
end;
